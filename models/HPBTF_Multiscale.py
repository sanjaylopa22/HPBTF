...
class Model(nn.Module):
    def __init__(self, configs, patch_lens=[16, 32], stride=8):
        super().__init__()
        ...
        self.multi_scale_patch_embedding = MultiScalePatchEmbedding(
        ...
        # Estimate number of patches based on smallest patch size
        smallest_patch_len = min(patch_lens)
        reduced_patches = int((configs.seq_len - smallest_patch_len) / stride + 2)
        reduced_patches = reduced_patches // (2 ** (2 - 1))  # Hierarchical level = 2
    ...
    def encode(self, x_enc):
        ...
        enc_out, n_vars = self.multi_scale_patch_embedding(x_enc)
        enc_out, _ = self.encoder(enc_out)
        enc_out = enc_out.view(-1, n_vars, enc_out.shape[1], enc_out.shape[2])
        return enc_out.permute(0, 1, 3, 2)  # [B, C, D, P]
